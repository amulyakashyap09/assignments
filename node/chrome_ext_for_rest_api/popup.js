...
function onOsInput(event) {
    if (!event.target.value) { console.error("Please select a OS"); } else {
...
function onTypeInput(event) {
    var type;
    if (!event.target.value) { console.err("Please select a Type"); } else {
...
function getOsBrowserList() {
    ...
    xhttp.setRequestHeader('Access-Control-Allow-Methods', 'OPTIONS,GET,PUT,POST,DELETE');
...
function onSubmit(event) {
    ...
    if (!type) { alert("Invalid Type!"); return false; } else if (!os) { alert("Invalid OS!"); return false; } else if (!browser_device) {
        if (type === "Desktop") { alert("Invalid Browser!"); } else { alert("Invalid Device!"); }
        return false;
...
function clearCookies(event) {
    ...
    chrome.cookies.getAllCookieStores(function(cookiestores) {
        for (i = 0; i < cookiestores.length; i++) {
            alert(JSON.stringify(cookiestores[i]));
        }
        console.log("cookies : ", cookiestores)
    })
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('button[type="button"]').onclick = clearCookies;
    // document.getElementById('cookie_button').onclick = clearCookies;

    getOsBrowserList(function(url) {
        getImageUrl(url, function(imageUrl, width, height) {}, function(errorMessage) {});
    });
});